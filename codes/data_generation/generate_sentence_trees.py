...
def balance_sample(syllogistic_tree):
    ...
    参数:
        syllogistic_tree: 字典,包含三段论树的信息
            ...
            - leaf_nodes_type_dic: 字典,记录叶子节点的类型
    ...
    leaf_nodes_type_dic = syllogistic_tree["leaf_nodes_type_dic"]
    leaf_nodes = syllogistic_tree["leaf_nodes"]
    count_dic = {x: len(all_nodes_type_dic[x]) for x in all_nodes_type_dic if len(leaf_nodes_type_dic[x]) > 0}
    ...
    chosen_node_idx = random.choice(range(len(leaf_nodes_type_dic[chosen_type])))
    chosen_node = leaf_nodes_type_dic[chosen_type].pop(chosen_node_idx)
...
if __name__ == "__main__":
    ...
    for i in range(args.num_trees):
        ...
        syllogistic_tree["leaf_nodes_type_dic"][conclusion_type].append(1)
